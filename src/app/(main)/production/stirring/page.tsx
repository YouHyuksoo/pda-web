/**
 * @file src/app/(main)/production/stirring/page.tsx
 * @description
 * 교반 페이지입니다. (HS605 대체)
 * 교반 시작/종료 시간을 기록합니다.
 *
 ...
 * 2. **바코드 스캔**: 처음 스캔 시 교반 시작, 다시 스캔 시 종료
 * 3. **저장**: 교반 기록 저장
 */
...
export default function StirringPage() {
  ...
  const [itemList, setItemList] = useState<StirringItem[]>([]);
  ...
  // 진행중/완료 카운트
  const stirringCount = itemList.filter((i) => i.status === 'stirring').length;
  ...
  const handleBarcodeScan = useCallback(
    (barcode: string) => {
      ...
      if (existingIdx >= 0) {
        ...
        const startTime = new Date(`${workDate} ${itemList[existingIdx].startTime}`);
        const endTime = new Date();
        const duration = Math.floor((endTime.getTime() - startTime.getTime()) / 60000); // 분 단위
        ...
        showScanSuccess(`${barcode} 종료 (${duration}분)`);
      } else {
        // 새 항목: 시작 처리
        const newItem: StirringItem = {
        ...
        showScanSuccess(`${barcode} 교반 시작`);
      }
    },
    [processCode, lineCode, itemList, workDate]
  );
  ...
  const handleSave = async () => {
    ...
    const confirmed = await showConfirm(
      ...
      { description: `진행중: ${stirringCount}건, 완료: ${completedCount}건` }
    );
    if (!confirmed) return;
    ...
    try {
      const response = await fetch('/api/production/stirring', {
        method: 'POST',
            ...
            <>
              <div className="flex justify-between items-center">
                ...
                <div className="flex gap-2">
                  <Badge variant="secondary">진행중: {stirringCount}</Badge>
                  <Badge variant="default">완료: {completedCount}</Badge>
                </div>
              </div>
                  ...
                  <TableBody>
                    {itemList.map((item, idx) => (
                      <TableRow
                        key={item.boxNo}
                        className={
                          item.status === 'completed' ? 'bg-green-50' : 'bg-orange-50'
                        }
                      >
                        ...
                        <TableCell>
                          ...
                          >
                            {item.status === 'completed' ? item.duration : '진행중'}
                          </Badge>
                        </TableCell>
                        <TableCell>
                          <Button
