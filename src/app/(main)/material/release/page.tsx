/**
 * @file src/app/(main)/material/release/page.tsx
 * @description
 * 자재출고 페이지 (HSJ120 대체)
 * 창고에서 자재를 출고 처리합니다.
 *
...
import { Save, Trash2, PackageX } from 'lucide-react';
...
export default function MaterialReleasePage() {
  ...
  const [itemList, setItemList] = useState<ReleaseItem[]>([]);
  const [isLoading, setIsLoading] = useState(false);

  useEffect(() => { setForm('HSJ120', '자재출고'); }, [setForm]);
  ...
  const handleSave = async () => {
    ...
    const confirmed = await showConfirm(`${itemList.length}건을 출고 처리하시겠습니까?`);
    if (!confirmed) return;

    setIsLoading(true);
    try {
      const response = await fetch('/api/material/release', {
        method: 'POST',
      ...
      if (result.success) {
        showSuccess(result.message || `${itemList.length}건 출고 처리되었습니다.`);
        setItemList([]);
      } else {
        ...
        <Card>
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <CardTitle className="text-base">출고 목록 ({itemList.length}건)</CardTitle>
